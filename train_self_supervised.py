...
from utils.caw_graph import CAWNeighborFinder
...
parser.add_argument('--pos_dim', type=int, default=100, help='shape of MLP encoding for each CAW sequence member(dimension of the positional embedding)')
...
parser.add_argument('--caw_layers', type=int, default=2, help='number of steps in CAW walks(when only 1 value passed in caw_neighbors)')
...
# TODO: create CAW neighbor finders here
if not IGNORE_CAW:
    ...
    caw_full_ngh_finder = CAWNeighborFinder(full_adj_list, bias=0.0, use_cache=False, sample_method='binary')
    ...
    for src, dst, eidx, ts in zip(val_data.sources, val_data.destinations, val_data.edge_idxs, val_data.timestamps):
        partial_adj_list[src].append((dst, eidx, ts))
        partial_adj_list[dst].append((src, eidx, ts))
    caw_partial_ngh_finder = CAWNeighborFinder(partial_adj_list, bias=0.0, use_cache=False, sample_method='binary')
    caw_ngh_finders = caw_partial_ngh_finder, caw_full_ngh_finder
...
for i in range(args.n_runs):
  ...
  # TODO: update model CAW neighbor finder here
  if not IGNORE_CAW:
    tgn.update_caw_ngh_finder(caw_partial_ngh_finder)
  ...
  for epoch in range(NUM_EPOCH):
    ...
    # TODO: update model CAW neighbor finder here
    if not IGNORE_CAW:
        tgn.update_caw_ngh_finder(caw_full_ngh_finder)
