...
from utils.caw_sample import *

PRECISION = 5


class CAWNeighborFinder:
    ...
    def find_before(self, src_idx, cut_time, e_idx=None, return_binary_prob=False):
        ...
        assert (len(neighbors_idx) == len(neighbors_ts) and len(neighbors_idx) == len(
            neighbors_e_idx))  # check the next line validality
        if e_idx is None:
        ...
        for i, (src_idx, cut_time) in enumerate(zip(src_idx_l, cut_time_l)):
            ngh_idx, ngh_eidx, ngh_ts, ngh_binomial_prob = self.find_before(src_idx, cut_time, e_idx=e_idx_l[
                i] if e_idx_l is not None else None,
                                                                            return_binary_prob=(
                                                                                        self.sample_method == 'binary'))  # TODO: change signature and content of the function
    ...
    def find_k_hop(self, k, src_idx_l, cut_time_l, num_neighbors, e_idx_l=None):
        ...
        return (node_records, eidx_records,
                t_records)  # each of them is a list of k numpy arrays, each in shape (batch,  num_neighbors ** hop_variable)
        ...
        if packed:
            ...
            for hop in range(len_walk):
                ...
                cur_n_idx, cur_time, cur_e_idx = n_idx_l[idx_picked], ts_l[idx_picked], e_idx_l[
                    idx_picked]  # update the query node info
            ...
            walks_n_idx, walks_e_idx, walks_ts = np.array(walk_n_idx, dtype=int), np.array(walk_e_idx,
                                                                                           dtype=int), np.array(walk_ts,
                                                                                                                dtype=float)
            return walks_n_idx, walks_e_idx, walks_ts
